...
// all-far frame or a single lit arc can never occur. Send crafted
// DISTANCE_SENSOR messages instead, onto the link rather than into the fact
// group: PoliceLidarMonitor listens to Vehicle's mavlinkMessageReceived, and
// handing the fact group a decoded message skips that dispatch entirely.
...
void ScreenshotTest::_captureScreens()
{
    ...
    runWithMockLink([] { return MockLink::startAPMArduCopterMockLink(MockConfiguration::OptionNone); },
        ...
        // Armed but nothing injected yet, so no DISTANCE_SENSOR has ever arrived and
        // the ring must stay blank: it is the sensor talking that raises it, not arming.
        vehicle->setArmed(true, false);
        QTRY_VERIFY(vehicle->armed());
        _grab(QStringLiteral("ring_neg_1_armed_no_telemetry"));
        if (QTest::currentTestFailed()) return;

        // Disarmed with the nose reading close, which one forward lidar draws: seeing it
        // work before takeoff is the point of the displays not waiting for arming.
        vehicle->setArmed(false, false);
        QTRY_VERIFY(!vehicle->armed());
        injectProximity(mockLink, vehicle, rgBad);
        ...
        injectProximity(mockLink, vehicle, rgFar);
        _grab(QStringLiteral("ring_1_all_far"));
        if (QTest::currentTestFailed()) return;

        injectProximity(mockLink, vehicle, rgWarn);
        ...
        injectProximity(mockLink, vehicle, rgMixed);
        ...
        for (int corner = 0; corner < 4; corner++) {
            ...
            injectProximity(mockLink, vehicle, rgOneClose);
